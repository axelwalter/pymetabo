...
mzML_dir = st.text_input("Path to input mzML file folder.", "/home/axel/Nextcloud/workspace/MetabolomicsWorkflowMayer/mzML")
...
if st.button("Run Workflow"):
    ...
    for i, mzML_file in enumerate(os.listdir(mzML_dir)):
        if not mzML_file.endswith("mzML"):
            continue
        FFM.run(os.path.join(mzML_dir, mzML_file), os.path.join(ffm_dir, mzML_file[:-4] + "featureXML"))
    ...
    ffm_table = pd.read_csv(os.path.join(ffm_consensus_dir, "FFM.tsv"), sep="\t")
    st.markdown("**Intermediate result:** Consensus features from FeatureFinderMetabo.")
    st.write(ffm_table)
